 ...
 */
public class NullThread implements Runnable {
  ThreadLocalRandom random = ThreadLocalRandom.current();
  Logger logger = LoggerFactory.getLogger(NullThread.class);

  @Override
  public void run() {
    int index = random.nextInt(0, VariateConcurrentUtils.nullVariate.size());
    Variate variate = VariateConcurrentUtils.nullVariate.get(index);
    ...
    try {
//      logger.info("null随机数为:{}",index);
//      logger.info("Null Operation");
//      VariateConcurrentUtils.utils.variateMapper.updateByExample(variate, example);
